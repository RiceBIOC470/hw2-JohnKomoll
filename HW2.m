...
% NOTICE: THIS CODE WILL RUN DIFFERENTLY THAN WHEN THE REPOSITORY WAS
% CREATED, BECAUSE meannonan.m WAS MODIFIED FOR PART 2 OF THIS PROBLEM.
...
% Mean is a vectorized built-in MATLAB function. It is designed to return
% the mean of each column for arrays, and the mean of a vector if given a
% single vector. yy is just the sum of the columns, as expected.
%
% However, zz is only a single number. This is because in the meannonan.m
% function, when an empty array is used to replace the NaN entry in the
% inputted matrix, the matrix becomes a row vector of each row in order.
% This is how MATLAB reacts to being given a matrix array with an empty
% array inside of it. Then, the mean function at the end only acts on a
% single vector and thus yields a single mean, as expected.
...
% See attached modified function
xx = rand(5); % random 5x5 matrix
xx(3,2) = NaN; %put a NaN in
yy = mean(xx); 
zz = meannonan(xx);
...
% See function generated in repository.
...
legend('N-ORF = 100', 'N-ORF = 200', 'N-ORF = 300', 'N-ORF = 400', 'Location', 'Best')
...
% See function in repository to find changes.
...
% See function written in repository.
